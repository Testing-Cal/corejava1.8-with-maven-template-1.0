                  ...
                  stage('SonarQube') {
                     ...
                     if (env.SONAR_CREDENTIAL_ID != null && env.SONAR_CREDENTIAL_ID != '') {
                         ...
                         withSonarQubeEnv('pg-sonar') {
                             sh '''docker run -v "$WORKSPACE":/app -w /app sonarsource/sonar-scanner-cli:11.0 -Dsonar.java.binaries='.' -Dsonar.exclusions='pom.xml, target/**/*' -Dsonar.projectKey="$sonar_project_key" -Dsonar.sources=src -Dsonar.projectName="$sonar_project_key" -Dsonar.organization="$sonar_org" -Dsonar.host.url="$SONAR_HOST_URL" -Dsonar.login=$SONAR_AUTH_TOKEN '''
                         }
                     }
                  }
